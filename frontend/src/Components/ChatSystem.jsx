...
function ChatSystem({ userInfo, onSignOut, onBackToSearch }) {
  ...
  return (
    <div className="chat-system">
      ...
      {/* Navigation */}
      <div className="chat-nav flex flex-col sm:flex-row gap-2 sm:gap-4">
        <button 
          onClick={() => setCurrentView('groups')}
          className={`nav-btn ${currentView === 'groups' ? 'active' : ''} text-sm sm:text-base px-3 sm:px-6 py-2 sm:py-3`}
        >
          My Groups
        </button>
        <button 
          ...
          className={`nav-btn ${currentView === 'discover' ? 'active' : ''} text-sm sm:text-base px-3 sm:px-6 py-2 sm:py-3`}
        >
          Discover Groups
        </button>
        <button 
          onClick={() => setCurrentView('create-group')}
          className={`nav-btn ${currentView === 'create-group' ? 'active' : ''} text-sm sm:text-base px-3 sm:px-6 py-2 sm:py-3`}
        >
          Create Group
        </button>
      </div>
              ...
              <div className="groups-grid">
                {groups.map(group => (
                  <div key={group.id} className="group-card">
                    ...
                    <div className="group-actions flex flex-col sm:flex-row gap-2">
                      <button 
                        onClick={() => openChat(group)}
                        className="btn btn-primary text-sm sm:text-base px-3 sm:px-4 py-2 w-full sm:w-auto"
                      >
                        Open Chat
                      </button>
                        ...
                        <>
                          {group.user_role === 'admin' && (
                            <button 
                              ...
                              className="btn btn-secondary text-sm sm:text-base px-3 sm:px-4 py-2 w-full sm:w-auto"
                            >
                              Edit
                            </button>
                          )}
                          <button 
                            ...
                            className="btn btn-secondary text-red-600 text-sm sm:text-base px-3 sm:px-4 py-2 w-full sm:w-auto"
                          >
                            Delete
                          </button>
                        </>
          ...
          <div className="chat-view">
            <div className="chat-header-info flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4">
              ...
              <div className="chat-group-info">
                <h2 className="text-lg sm:text-xl font-semibold">{selectedGroup.name}</h2>
                <p className="text-xs sm:text-sm text-gray-600">{selectedGroup.member_count} members</p>
              </div>
            </div>

            <div className="messages-container" style={{ maxHeight: window.innerWidth < 640 ? '50vh' : '70vh' }} role="log" aria-live="polite" aria-label="Chat messages">
              {loading ? (
            ...
            <form onSubmit={sendMessage} className="message-form" aria-label="Send message form">
              <div className="message-input-container flex flex-col sm:flex-row gap-2 sm:gap-3">
                ...
                <input
                  ...
                  className="message-input flex-1 text-base px-4 py-3 min-h-[44px]"
                ...
                <button 
                  type="submit" 
                  className="btn btn-primary text-base px-6 py-3 min-h-[44px] w-full sm:w-auto"
